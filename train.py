...
parser.add_argument("--train_path", default="./data1/train.txt", type=str, help="The path of train set.")
parser.add_argument("--dev_path", default="./data1/test.txt", type=str, help="The path of dev set.")
...
def train():
    with open('data/logs.txt', 'w', encoding='utf-8') as ins_test_file:
        reset_console()
        # model = AutoModelForSequenceClassification.from_pretrained(args.model, num_labels=args.num_labels)
        # tokenizer = AutoTokenizer.from_pretrained(args.model)
        saved_model_path = 'data/checkpoints/model_best_3'  # 训练模型存放地址
        ...
        for epoch in range(1, args.num_train_epochs+1):
            for batch in train_dataloader:
                ...
                if global_step % args.logging_steps == 0:
                    ...
                    str_temp = "global step %d, epoch: %d, loss: %.9f, speed: %.2f step/s" \
                             % (global_step, epoch, loss_avg, args.logging_steps / time_diff)
                    print(str_temp)
                    ins_test_file.write(str_temp + '\n')
                    tic_train = time.time()
                ...
                if global_step % args.valid_steps == 0:
                    ...
                    ins_test_file.write("Evaluation precision: %.5f, recall: %.5f, F1: %.5f" % (precision, recall, f1)
                                        + '\n')
                    if f1 > best_f1:
                        print(
                        ...
                        ins_test_file.write(f"best F1 performence has been updated: {best_f1:.5f} --> {f1:.5f}"+'\n'
